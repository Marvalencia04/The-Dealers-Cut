...
/// - Genera apuestas aleatorias con m�nimo (fase 1)
...
public class BlackjackNPCManager : MonoBehaviour
{
    ...
    [Header("Apuestas")]
    [Tooltip("Multiplicador m�ximo de la apuesta m�nima para generar apuestas aleatorias.")]
    [SerializeField] private int maxBetMultiplier = 6;

    [Tooltip("Variaci�n adicional (0..1) aplicada a la apuesta para que no sea tan uniforme.")]
    [Range(0f, 1f)]
    [SerializeField] private float betNoise = 0.25f;

    [Header("IA HIT/STAND")]
    [Tooltip("Puntuaci�n base a partir de la cual el NPC tiende a plantarse (se ajusta con riesgo).")]
    ...
    // ------------------------------------------------------------
    // API p�blica (la usa BlackjackTable / RondaManager)
    // ------------------------------------------------------------
    ...
    /// <summary>
    /// Fase 1: genera apuestas aleatorias con m�nimo (seg�n el d�a).
    /// En el GDD: los jugadores apuestan cantidad aleatoria con apuesta m�nima. :contentReference[oaicite:4]{index=4}
    /// </summary>
    ...
    public void StartNPCsTurn()
    {
        if (table == null)
        {
           // Debug.LogError("[BlackjackNPCManager] No hay tabla v�lida.");
    ...
    private IEnumerator NPCsTurnRoutine()
    {
        foreach (var npc in npcs)
        {
            ...
            // Si ya est� plantado por alg�n motivo, saltar
    ...
    private IEnumerator SingleNPCTurn(BlackjackNPC npc)
    {
        while (table.IsRoundActive)
        {
            ...
            // Riesgo alto => se planta m�s tarde (pide m�s).
            ...
            {
                // cuanto m�s lejos de threshold, m�s probable HIT
